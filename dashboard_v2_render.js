...
/* =====================================================================
   VALEURS RELATIVES — la part de chaque marque, lisible sans survol
   ---------------------------------------------------------------------
   L'infobulle exige un geste et ne montre qu'une valeur a la fois. Pour
   comparer des parts, le lecteur doit pouvoir les lire toutes d'un coup.
   Ces plugins inscrivent donc le POURCENTAGE directement sur la marque.

   Repartition des roles, pour ne pas surcharger :
     - etiquette directe  -> la part (%), ce qui se compare
     - infobulle          -> le montant exact en francs
     - « Voir les chiffres » -> le tableau complet

   Les courbes en sont exclues : une etiquette sur chaque point d'une
   serie temporelle produit un mur de chiffres illisible.
   ===================================================================== */


/* Part de chaque secteur, inscrite dans l'anneau. */
const pctAnneau = () => ({
  id: 'pctA' + Math.random().toString(36).slice(2, 7),
  afterDatasetsDraw(ch) {
    const meta = ch.getDatasetMeta(0);
    const data = ch.data.datasets[0].data;
    const total = data.reduce((a, b, i) =>
      a + (ch.getDataVisibility(i) ? (b || 0) : 0), 0);
    if (!total) return;
    const ctx = ch.ctx;
    ctx.save();
    ...
    meta.data.forEach((arc, i) => {
      if (!ch.getDataVisibility(i)) return;
      const pct = 100 * (data[i] || 0) / total;
      // Un secteur trop fin ne peut pas contenir son etiquette.
      if (pct < 4) return;
      const {x, y} = arc.getCenterPoint();
      // Encre sombre sur les teintes claires, claire sur les sombres :
      // une etiquette blanche sur jaune serait illisible.
      const fond = ch.data.datasets[0].backgroundColor[i] || '';
      ctx.fillStyle = /^#(fab219|c98500|eda100|b7d3f6|86b6ef)/i.test(fond)
        ? '#001A20' : '#FFFFFF';
      ctx.fillText(F1(pct) + ' %', x, y);
    });
    ctx.restore();
  },
});

/* Ajoute la part au texte d'une infobulle, sans remplacer le montant. */
...
const etiquettesPlugin = (fmt, axe, alterner, base) => ({
    ...
    ch.data.datasets.forEach((ds,di)=>{
      ...
      meta.data.forEach((el,i)=>{
        ...
        if(axe==='y'){ ctx.textAlign='left'; ctx.textBaseline='middle';
          ...
          if(base){
            // Deux lignes : le montant, puis sa part juste dessous. La
            // part n'est PAS dessinee sous l'axe, ou elle recouvrirait
            // les noms d'etapes deja ecrits par Chart.js.
            ctx.fillText(fmt(v), el.x, el.y+dy-11);
            const police = ctx.font, encre = ctx.fillStyle;
            ctx.font = "10px 'Inter',sans-serif"; ctx.fillStyle = C.dim;
            ctx.fillText(F1(100*Math.abs(v)/base)+' %', el.x, el.y+dy);
            ctx.font = police; ctx.fillStyle = encre;
          } else {
...
RENDER['synthese'] = function(){
  ...
  // Pre-agrege au grain jour : la Synthese n'a besoin que du total
  // offert, pas du detail par caissier et categorie (484 Ko).
  const off = inRange(DATA.offerts_total_jour, state.start, state.end);
  ...
  setChart('c-sy-casc', {
    ...
    plugins:[etiquettesPlugin(v=>Fc(Math.abs(v)), 'x', true, t.cn)]
  });
  ...
  // Pre-agrege au grain jour : deux colonnes suffisent pour ce taux,
  // la place de parcourir horaire_jour et ses 1,8 Mo.
  const h = inRange(DATA.nuit_jour, state.start, state.end);
  const totH = sum(h,'cnt');
  const nuit = sum(h,'cnn');
...
RENDER['resultat'] = function(){
  ...
  setChart('c-re-rn', {type:'bar',
    ...
    // Part du resultat net dans le CA net DU MOIS : c'est le taux de
    // rentabilite mensuel, plus parlant qu'une part du total annuel.
    plugins:[{id:'rnpct', afterDatasetsDraw(ch){
      const ctx=ch.ctx; ctx.save();
      ctx.font="600 10.5px 'Inter',sans-serif"; ctx.fillStyle=C.ink;
      ctx.textAlign='center'; ctx.textBaseline='bottom';
      ch.getDatasetMeta(0).data.forEach((el,i)=>{
        const k=pm.ks[i], g=pm.get(k);
        if(!g || !g.cn) return;
        ctx.fillText(F1(PCT(g.rn,g.cn))+' %', el.x, el.y-5);
      });
      ctx.restore();
    }}]});
...
/* =====================================================================
   ...
   ===================================================================== */
/* =====================================================================
   PAGE — EXPLORER LES VENTES
   ---------------------------------------------------------------------
   Trois niveaux : activite -> famille -> produit. Les trois s'agregent
   depuis UN SEUL jeu de donnees, articles_mois, pour que les totaux
   concordent d'un niveau a l'autre. Melanger deux sources de
   granularites differentes (le jour pour les categories, le mois pour
   les produits) les ferait diverger de quelques francs, et un ecart
   inexplique entre deux ecrans detruit la confiance dans l'ensemble.

   Consequence assumee : cette page raisonne en MOIS ENTIERS, alors que
   le reste du tableau de bord filtre au jour. C'est dit dans le bandeau
   de la page, pas seulement ici.
   ===================================================================== */

/* Lignes du mois, selon le filtre de periode. On retient tout mois
   compris entre le mois de debut et le mois de fin. */
function exploLignes(){
  const m1 = state.start.slice(0,7), m2 = state.end.slice(0,7);
  return DATA.articles_mois.filter(r => r.m >= m1 && r.m <= m2);
}

function exploNiveau(){
  const e = state.explo;
  if(!e.t) return 'activite';
  if(!e.c) return 'famille';
  return 'produit';
}

RENDER['explorer'] = function(){
  const e = state.explo;
  const toutes = exploLignes();

  // Perimetre courant, selon la profondeur
  let lignes = toutes;
  if(e.t) lignes = lignes.filter(r => r.t === e.t);
  if(e.c) lignes = lignes.filter(r => r.c === e.c);

  const niveau = exploNiveau();
  const cleGroupe = niveau === 'activite' ? 't' : (niveau === 'famille' ? 'c' : 'a');
  const nomNiveau = niveau === 'activite' ? 'activité'
                  : (niveau === 'famille' ? 'famille' : 'produit');
  // Ce libelle nomme ce qui est effectivement COMPTE a l'ecran, c'est-a-dire
  // le niveau de regroupement courant -- et non le niveau suivant. La
  // premiere version affichait « PRODUITS 19 » sur un ecran qui montrait
  // 19 familles.
  const nomCompte = niveau === 'activite' ? 'activités'
                  : (niveau === 'famille' ? 'familles' : 'produits');

  /* ---------- fil d'Ariane ---------- */
  let fil = `<a data-vers="racine">Toutes les activités</a>`;
  if(e.t){
    fil += `<span class="sep">›</span>`;
    fil += e.c ? `<a data-vers="activite">${e.t}</a>`
               : `<span class="ici">${e.t}</span>`;
  }
  if(e.c) fil += `<span class="sep">›</span><span class="ici">${e.c}</span>`;
  document.getElementById('ex-fil').innerHTML = fil;
  document.querySelectorAll('#ex-fil a').forEach(a=>{
    a.addEventListener('click', ()=>{
      const v = a.dataset.vers;
      state.explo = v === 'racine' ? {t:null, c:null} : {t:state.explo.t, c:null};
      render('explorer');
    });
  });

  /* ---------- totaux du perimetre ---------- */
  const tot = sums(lignes, ['q','cn','cb','rm']);
  const totGeneral = sum(toutes, 'cn');
  const nbEnfants = new Set(lignes.map(r => r[cleGroupe])).size;
  ...
  document.getElementById('ex-kpi').innerHTML = [
    ...
    tuile({k:nomCompte.toUpperCase(),
           v:F(nbEnfants),
           d:niveau==='produit'
             ? 'dernier niveau : les produits ne se déplient pas davantage'
             : 'cliquez une ligne pour descendre'}),
  ].join('');

  /* ---------- repartition du niveau ---------- */
  const groupes = [...groupBy(lignes, r=>r[cleGroupe], ['q','cn','cb','rm'])]
    .map(([k,v]) => ({k, ...v}))
    .sort((a,b) => b.cn - a.cn);
  const affiches = groupes.slice(0, 18);

  const couleurNiveau = niveau === 'activite'
    ? affiches.map(g => C[g.k] || C.seq[4])
    : (e.t && C[e.t] ? C[e.t] : C.seq[4]);

  document.getElementById('ex-titre-graph') .textContent =
    niveau === 'produit' ? 'Produits de la famille' :
    (niveau === 'famille' ? `Familles de ${e.t}` : 'Les trois activités');
  document.getElementById('ex-sous-graph').textContent =
    niveau === 'produit'
      ? `${groupes.length} produits, les 18 premiers affichés.`
      : `Cliquez une barre pour descendre au niveau ${niveau==='activite'?'famille':'produit'}.`;

  setChart('c-ex-parts', {type:'bar',
    data:{labels:affiches.map(g=>g.k), datasets:[Object.assign({},BAR,{
      label:'CA net', data:affiches.map(g=>g.cn),
      backgroundColor:couleurNiveau, maxBarThickness:niveau==='activite'?54:18})]},
    options:{indexAxis:'y',
      // Marge a droite : l'etiquette de pourcentage est dessinee APRES la
      // barre, et celle de la plus longue sortait de la zone de trace.
      layout:{padding:{right:46}},
      // Le clic sur une barre descend d'un niveau : c'est le meme geste
      // que le clic sur une ligne du tableau, pour ne pas avoir a deviner
      // lequel des deux est actif.
      onClick:(evt, elements)=>{
        if(!elements.length || niveau === 'produit') return;
        descendre(affiches[elements[0].index].k);
      },
      onHover:(evt, elements)=>{
        evt.native.target.style.cursor =
          (elements.length && niveau !== 'produit') ? 'pointer' : 'default';
      },
      plugins:{legend:{display:false}, tooltip:Object.assign({},TOOLTIP,{callbacks:{
        label:c=>{const g=affiches[c.dataIndex];
          return [' CA net : '+FCFA(g.cn),
                  ' '+F1(PCT(g.cn, tot.cn))+' du périmètre affiché',
                  ' Quantité : '+F(g.q)+' · prix moyen '+F(g.cn/(g.q||1))+' F'];}}})},
      scales:{x:axisY(), y:{grid:{display:false}, border:{color:C.axis},
        ticks:{color:C.dim, font:{size:niveau==='activite'?12:10.5}, autoSkip:false}}}},
    plugins:[pctBarres('y')]});

  document.getElementById('ex-alt').innerHTML = tableHTML(
    [{t:nomNiveau.charAt(0).toUpperCase()+nomNiveau.slice(1)},
     {t:'CA net',num:true},{t:'Part',num:true},{t:'Quantité',num:true}],
    groupes.map(g=>[g.k, F(g.cn), F1(PCT(g.cn, tot.cn))+' %', F(g.q)]));

  /* ---------- evolution mensuelle du perimetre ---------- */
  const parMoisExplo = groupBy(lignes, r=>r.m, ['cn']);
  const mois = [...parMoisExplo.keys()].sort();
  setChart('c-ex-evol', {type:'line',
    data:{labels:mois.map(m=>MOIS[parseInt(m.slice(5,7),10)-1]+' '+m.slice(2,4)),
      datasets:[Object.assign({},LINE,{
        label:'CA net', data:mois.map(m=>parMoisExplo.get(m).cn),
        borderColor:e.t?C[e.t]:C.accentMark,
        backgroundColor:'rgba(15,163,191,.14)',
        pointBackgroundColor:e.t?C[e.t]:C.accentMark, fill:true})]},
    options:{interaction:{mode:'index',intersect:false},
      plugins:{legend:{display:false}, tooltip:Object.assign({},TOOLTIP,{
        callbacks:{label:c=>' CA net : '+FCFA(c.parsed.y)}})},
      scales:{y:axisY(), x:axisX()}}});
  document.getElementById('ex-sous-eval').textContent =
    `${e.c || e.t || 'Toutes activités'} — ${mois.length} mois affichés.`;

  /* ---------- tableau du niveau ---------- */
  document.getElementById('ex-titre-table').textContent =
    niveau === 'produit' ? `Produits de ${e.c}` :
    (niveau === 'famille' ? `Familles de ${e.t}` : 'Les trois activités');
  document.getElementById('ex-sous-table').textContent =
    niveau === 'produit'
      ? 'Dernier niveau : ces lignes ne se déplient pas davantage.'
      : 'Cliquez une ligne pour descendre.';

  const lignesTable = groupes.map((g,i)=>[
    String(i+1),
    niveau === 'activite' ? pill(g.k, g.k.toLowerCase())
                          : `<span class="strong">${g.k}</span>`,
    F(g.cn), F1(PCT(g.cn, tot.cn))+' %', F(g.q),
    F(g.cn/(g.q||1)), F(g.rm), F1(PCT(g.rm, g.cb||g.cn))+' %',
  ]);
  document.getElementById('ex-table').innerHTML = tableHTML(
    [{t:'#'},{t:nomNiveau.charAt(0).toUpperCase()+nomNiveau.slice(1)},
     {t:'CA net',num:true},{t:'Part',num:true},{t:'Quantité',num:true},
     {t:'Prix moyen',num:true},{t:'Remises',num:true},{t:'Taux remise',num:true}],
    lignesTable);

  if(niveau !== 'produit'){
    document.querySelectorAll('#ex-table tbody tr').forEach((tr,i)=>{
      tr.classList.add('cliquable');
      tr.addEventListener('click', ()=> descendre(groupes[i].k));
    });
  }
  document.getElementById('ex-fin').textContent = niveau === 'produit'
    ? 'Le produit est le dernier niveau disponible. Pour le coût de revient de chaque produit, voir la page « Coût de revient ».'
    : '';

  rendreFiltrable('ex-table', niveau === 'activite'
    ? 'Filtrer…' : (niveau === 'famille' ? 'Filtrer une famille…' : 'Filtrer un produit…'));
};

/* Descend d'un niveau. La page se redessine entierement : il n'y a
   qu'un seul chemin de rendu, donc aucun risque qu'un etat intermediaire
   reste affiche. */
function descendre(valeur){
  const e = state.explo;
  if(!e.t) state.explo = {t:valeur, c:null};
  else if(!e.c) state.explo = {t:e.t, c:valeur};
  else return;
  render('explorer');
  document.getElementById('page-explorer').scrollIntoView({block:'start'});
}
...
RENDER['ventes'] = function(){
  ...
  setChart('c-ve-top', {type:'bar',
    ...
    // Part dans le CA TOTAL, pas dans le total des 15 affiches : c'est
    // le poids reel de l'article dans l'activite.
    plugins:[{id:'toppct', afterDatasetsDraw(ch){
      const ctx=ch.ctx; ctx.save();
      ctx.font="600 10.5px 'Inter',sans-serif"; ctx.fillStyle=C.ink;
      ctx.textAlign='left'; ctx.textBaseline='middle';
      ch.getDatasetMeta(0).data.forEach((el,i)=>{
        ctx.fillText(F2(PCT(top[i].cn, totArtCn))+' %', el.x+7, el.y);
      });
      ctx.restore();
    }}]});
...
function dessinerCatalogue(){
  ...
  host.innerHTML = h;
  rendreFiltrable('ve-table', 'Filtrer : SHISHA, CHAMPAGNE, BIERES, DRINK…');
...
RENDER['cout-revient'] = function(){
  ...
  rendreFiltrable('cr-table', 'Filtrer : BURGER, GASTRONOMIE, cuisine, partiel…');
};
...
RENDER['depenses'] = function(){
  ...
  rendreFiltrable('de-table', 'Filtrer : LOYERS, OPEX, CAPEX, DRINK…');
};
...
RENDER['qualite'] = function(){
  ...
  const vol = [
    ...
    ['Détail des dépenses', F(DATA.volumetrie['depenses_jour'])+' lignes', 'saisie manuelle quotidienne',
     'achats, charges, investissements'],
    ['Articles offerts', F(DATA.volumetrie['offerts_jour'])+' lignes', 'grain jour × caissier × catégorie',
