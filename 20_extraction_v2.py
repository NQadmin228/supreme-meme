...
# Grain MOIS x activite x categorie x article, pour l'exploration
# hierarchique (activite -> famille -> produit).
#
# Pourquoi le mois et non le jour : mesure sur ce jeu de donnees,
#   jour  x article -> 59 846 lignes, environ 4,8 Mo, le fichier doublerait
#   MOIS  x article ->  5 871 lignes, environ 0,5 Mo
# Une exploration par produit n'a pas besoin d'une granularite au jour :
# le mois suffit a suivre une tendance, et le poids reste negligeable.
# Les trois niveaux de l'exploration s'agregent depuis CE SEUL jeu, de
# sorte que les totaux concordent d'un niveau a l'autre -- melanger deux
# sources de granularites differentes les ferait diverger.
v['mois'] = v['jour_exploitation'].dt.strftime('%Y-%m')
am = v.groupby(['mois', 'Type', 'Categorie', 'Articles'], as_index=False).agg(
    q=('Qté', 'sum'), cn=('CAF', 'sum'), cb=('ca_brut_ligne', 'sum'),
    rm=('Remise', 'sum'))
articles_mois = [
    {'m': r.mois, 't': r.Type, 'c': r.Categorie, 'a': r.Articles,
     'q': round(r.q), 'cn': round(r.cn), 'cb': round(r.cb), 'rm': round(r.rm)}
